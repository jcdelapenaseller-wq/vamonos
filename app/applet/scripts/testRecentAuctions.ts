...
async function run() {
  ...
  try {
    ...
    await page.evaluate(() => {
      const desdeInput = document.querySelector('input[name="dato[18][0]"]') as HTMLInputElement;
      const hastaInput = document.querySelector('input[name="dato[18][1]"]') as HTMLInputElement;
    ...
    await Promise.all([
      ...
      page.evaluate(() => {
        const btn = document.querySelector('input[type="submit"][value="Buscar"]') as HTMLInputElement;
        if (btn) btn.click();
      })
    ]);
